      ...
      <div className="flex h-full max-h-[92vh] w-full max-w-5xl flex-col rounded-2xl bg-white shadow-2xl overflow-hidden border border-slate-200">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 bg-surface-card px-6 py-4 text-white">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-secondary/80 font-black text-deep-green shadow-sm">
        ...
        {/* Stepper Wizard Bar */}
        <div className="flex items-center justify-between border-b border-slate-200 bg-bg-secondary px-6 py-2.5 text-xs font-semibold text-slate-600 overflow-x-auto">
          <div className={`flex items-center gap-2 ${currentStep === 'UPLOAD' ? 'text-amber-600 font-bold' : ''}`}>
            <span className={`flex h-5 w-5 items-center justify-center rounded-full text-[10px] ${currentStep === 'UPLOAD' ? 'bg-secondary text-white' : 'bg-slate-200 text-slate-700'}`}>1</span>
            <span>Upload File</span>
          </div>
          <span className="text-slate-300">→</span>
          <div className={`flex items-center gap-2 ${currentStep === 'MAP' ? 'text-amber-600 font-bold' : ''}`}>
            <span className={`flex h-5 w-5 items-center justify-center rounded-full text-[10px] ${currentStep === 'MAP' ? 'bg-secondary text-white' : 'bg-slate-200 text-slate-700'}`}>2</span>
            <span>Map Columns</span>
          </div>
          <span className="text-slate-300">→</span>
          <div className={`flex items-center gap-2 ${currentStep === 'PREVIEW' ? 'text-amber-600 font-bold' : ''}`}>
            <span className={`flex h-5 w-5 items-center justify-center rounded-full text-[10px] ${currentStep === 'PREVIEW' ? 'bg-secondary text-white' : 'bg-slate-200 text-slate-700'}`}>3</span>
            <span>Validation & Duplicate Check</span>
          </div>
          <span className="text-slate-300">→</span>
          <div className={`flex items-center gap-2 ${currentStep === 'RESULT' ? 'text-amber-600 font-bold' : ''}`}>
            <span className={`flex h-5 w-5 items-center justify-center rounded-full text-[10px] ${currentStep === 'RESULT' ? 'bg-deep-teal text-white' : 'bg-slate-200 text-slate-700'}`}>4</span>
            <span>Execution Summary</span>
          </div>
        </div>

        {/* Step Body */}
        <div className="flex-1 overflow-y-auto p-6 bg-bg-secondary/50">
          {/* ================= STEP 1: UPLOAD ================= */}
          {currentStep === 'UPLOAD' && (
            <div className="space-y-6">
                    ...
                    return (
                      <button
                        ...
                        className={`flex flex-col items-start p-3 rounded-xl border text-left transition-all ${
                          isSelected
                            ? 'bg-amber-50 border-amber-400 ring-2 ring-amber-400/20 text-deep-green shadow-sm'
                            : 'bg-white border-slate-200 text-slate-600 hover:border-slate-300 hover:bg-bg-secondary'
                        }`}
                      >
              ...
              <div
                ...
                className={`relative flex flex-col items-center justify-center rounded-2xl border-2 border-dashed p-8 transition-colors ${
                  isDragOver ? 'border-secondary bg-amber-50/50' : 'border-slate-300 bg-white hover:border-amber-400'
                }`}
              >
                <input
                  id={fileInputId}
                ...
                <h4 className="text-sm font-bold text-text-primary mb-1">Drop your CSV / Excel export file here</h4>
                ...
                <label
                  htmlFor={fileInputId}
                  className="cursor-pointer inline-flex items-center gap-2 rounded-xl bg-surface-card px-5 py-2.5 text-xs font-bold text-white shadow-md hover:bg-slate-800 transition-all"
                >
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 rounded-xl border border-amber-200 bg-amber-50/70 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="text-xs font-bold text-text-primary">Need the exact formatted structure?</div>
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-lg border border-amber-400 bg-white px-3 py-1.5 text-xs font-bold text-text-primary shadow-xs hover:bg-amber-100/50 self-end sm:self-auto shrink-0"
                >
          ...
          {currentStep === 'MAP' && (
            <div className="space-y-5">
              <div className="flex items-center justify-between bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
                <div>
                  <h4 className="text-sm font-bold text-text-primary">Map File Columns to Master Schema</h4>
              ...
              <div className="rounded-xl border border-slate-200 bg-white overflow-hidden shadow-xs">
                ...
                <div className="divide-y divide-slate-100 max-h-[46vh] overflow-y-auto">
                  {currentSchema.fields.map((field) => {
                    ...
                    return (
                      <div key={field.key} className="grid grid-cols-12 items-center p-3 text-xs hover:bg-bg-secondary">
                        ...
                        <div className="col-span-3 truncate text-slate-500 font-mono text-[11px] bg-bg-secondary px-2 py-1 rounded">
                          {sampleVal}
                        </div>
                      </div>
                    );
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="rounded-xl border border-slate-200 bg-white p-3.5 shadow-xs">
                  ...
                  <div className="text-xl font-black text-deep-green">{validationSummary.totalRows}</div>
                </div>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-white p-3.5 rounded-xl border border-slate-200">
                <div className="flex items-center gap-2 text-xs font-bold text-slate-700">
                  ...
                  <select
                    ...
                    className="rounded-lg border border-slate-300 bg-bg-secondary px-2.5 py-1 text-xs font-bold text-text-primary"
                  >
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                  <div className="relative flex-1 sm:w-56">
                    ...
                    <input
                      ...
                      className="w-full rounded-lg border border-slate-200 bg-bg-secondary pl-8 pr-2.5 py-1 text-xs"
                    />
                  </div>
                    ...
                    {(['ALL', 'VALID', 'ERROR', 'DUPLICATE'] as const).map((filter) => (
                      <button
                        ...
                        className={`px-2 py-0.5 rounded-md transition-colors ${
                          previewFilter === filter ? 'bg-white text-text-primary shadow-xs' : 'text-slate-600 hover:text-text-primary'
                        }`}
                      >
                        {filter}
                      </button>
                              ...
                              <td className="px-3 py-2">
                                {!r.isValid ? (
                                  <div className="group relative inline-flex items-center gap-1 text-rose-700 font-bold text-[10px] bg-rose-100 px-1.5 py-0.5 rounded cursor-help">
                                    ...
                                    <span>Error</span>
                                    <div className="absolute left-0 bottom-full mb-1 hidden w-60 rounded bg-surface-card p-2 text-[10px] text-white shadow-lg group-hover:block z-20">
                                      {r.errors.join(', ')}
                                    </div>
                                  </div>
                                ) : r.isDuplicate ? (
          ...
          {currentStep === 'IMPORTING' && (
            <div className="flex flex-col items-center justify-center py-16 text-center space-y-4">
              ...
              <h4 className="text-lg font-bold text-text-primary">Importing {selectedEntity} Records...</h4>
          ...
          {currentStep === 'RESULT' && validationSummary && (
            <div className="space-y-6">
              <div className="flex items-center gap-4 bg-emerald-50 border border-emerald-200 rounded-2xl p-6">
                <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-secondary text-white shadow-md">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-xs">
                  ...
                  <div className="text-2xl font-black text-deep-green">{validationSummary.totalRows}</div>
                </div>
          ...
          <div>
            {currentStep === 'UPLOAD' ? (
              <button
                type="button"
                onClick={onClose}
                className="rounded-xl border border-slate-200 px-4 py-2 text-xs font-bold text-slate-600 hover:bg-bg-secondary"
              >
                Cancel
              </button>
            ) : currentStep === 'RESULT' ? (
              <button
                type="button"
                onClick={handleReset}
                className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 px-4 py-2 text-xs font-bold text-slate-700 hover:bg-bg-secondary"
              >
                <RefreshCw className="h-3.5 w-3.5" />
          ...
          <div>
            {currentStep === 'MAP' && (
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl bg-surface-card px-5 py-2 text-xs font-bold text-white shadow-md hover:bg-slate-800"
              >
                <span>Validate & Duplicate Check</span>
                <ArrowRight className="h-4 w-4 text-deep-teal" />
              </button>
            )}
            ...
            {currentStep === 'PREVIEW' && validationSummary && (
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl bg-secondary px-5 py-2 text-xs font-bold text-deep-green shadow-md hover:bg-secondary/80 disabled:opacity-50"
              >
                <span>Confirm & Import ({validationSummary.validRows + (duplicateStrategy === 'UPDATE' ? validationSummary.duplicateRows : 0)} Records)</span>
                <Database className="h-4 w-4 text-deep-green" />
              </button>
            )}
            ...
            {currentStep === 'RESULT' && (
              <button
                type="button"
                onClick={onClose}
                className="rounded-xl bg-surface-card px-5 py-2 text-xs font-bold text-white shadow-md hover:bg-slate-800"
              >
                Done & Close
              </button>
            )}
