...
export const GlobalSearchModal: React.FC<GlobalSearchModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-surface-card/60 p-4 pt-16 sm:pt-24 backdrop-blur-xs">
      <div className="flex w-full max-w-3xl flex-col rounded-2xl bg-white shadow-2xl overflow-hidden border border-slate-200">
        ...
        <div className="relative flex items-center border-b border-slate-200 bg-white px-4 py-3.5">
          <Search className="h-5 w-5 text-deep-teal shrink-0" />
          <input
            ref={inputRef}
            type="text"
            ...
            className="ml-3 flex-1 bg-transparent text-sm font-medium text-text-primary placeholder:text-slate-400 focus:outline-hidden"
          />
          {query && (
            <button
              onClick={() => setQuery('')}
        ...
        <div className="flex items-center gap-1.5 border-b border-slate-100 bg-bg-secondary/80 px-4 py-2 text-xs font-semibold overflow-x-auto">
          {(['ALL', 'CUSTOMERS', 'SKUS', 'INVOICES', 'ORDERS', 'DISPATCHES'] as const).map((cat) => (
            <button
              ...
              className={`rounded-lg px-2.5 py-1 transition-colors whitespace-nowrap ${
                activeCategory === cat
                  ? 'bg-primary text-deep-green hover:bg-primary/90 font-bold shadow-xs'
                  : 'text-slate-600 hover:bg-slate-200/70 hover:text-text-primary'
              }`}
            >
              ...
              return (
                <div
                  ...
                  className={`flex items-center justify-between p-3 rounded-xl cursor-pointer transition-colors ${
                    isSelected ? 'bg-amber-50/80 border border-amber-300/80' : 'hover:bg-bg-secondary'
                  }`}
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-slate-700">
                      {item.category === 'CUSTOMERS' && <Building2 className="h-4 w-4 text-deep-teal" />}
                      {item.category === 'SKUS' && <Package className="h-4 w-4 text-amber-600" />}
                      {item.category === 'INVOICES' && <FileText className="h-4 w-4 text-deep-teal" />}
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="truncate text-xs font-bold text-text-primary">{item.title}</span>
        ...
        {/* Footer shortcuts */}
        <div className="flex items-center justify-between border-t border-slate-100 bg-bg-secondary px-4 py-2 text-[11px] text-slate-500">
