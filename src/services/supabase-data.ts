...
export async function loadSupabaseAppData(currentUser: User): Promise<SupabaseAppData> {
  ...
  const [customerResult, skuResult, balanceResult, orderResult, invoiceResult, recoveryResult, ledgerResult, dispatchResult, returnResult, visitResult] = await Promise.all([
    ...
    db.from('dispatches').select('*,invoices(invoice_code),transporters(name),vehicles(registration_no),drivers(name,mobile),addas(name)').order('dispatch_date', { ascending: false }),
  ...
  const dispatches: Dispatch[] = (dispatchResult.data || []).map((r: any) => ({
    ...
    warehouseId: '',
    warehouseName: '',
    transporterName: r.transporters?.name || '',
    vehicleNumber: r.vehicles?.registration_no || '',
    driverName: r.drivers?.name || '',
    driverPhone: r.drivers?.mobile || '',
    addaName: r.addas?.name || undefined,
    bilityNumber: undefined,
