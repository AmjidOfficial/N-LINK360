...
export async function syncDatabaseToGoogleSheet(
  ...
  const salesRows: any[][] = [
    [
      ...
      'Payment Mode',
      'Approval Status',
      'Executive Approver (Shahzad Ullah)',
      'Order Status',
      'Remarks',
      'Last Synced At',
    ],
    ...appData.salesOrders.map((o: any) => {
      ...
      return [
        ...
        o.status === 'APPROVED' ? 'APPROVED' : 'PENDING_APPROVAL',
  ...
  const recoveryRows: any[][] = [
    [
      ...
      'Collection Status',
      'Confirmation Status',
      'Executive Confirmer (Shahzad Ullah)',
      'Last Synced At',
    ],
    ...appData.recoveries.map((r: any) => {
      ...
      return [
        ...
        r.status || 'COLLECTED',
        r.status === 'VERIFIED' ? 'CONFIRMED' : 'PENDING_CONFIRMATION',
...
export async function pushOrderToGoogleSheet(
  ...
  const row = [
    ...
    order.status === 'APPROVED' ? 'APPROVED' : 'PENDING_APPROVAL',
...
export async function pushRecoveryToGoogleSheet(
  ...
  const row = [
    ...
    recovery.status === 'VERIFIED' ? 'CONFIRMED' : 'PENDING_CONFIRMATION',
