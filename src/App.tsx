...
export default function App() {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      {/* ⚡ Dynamic Role Switcher Dock */}
      <div className="bg-slate-900 text-slate-100 border-b border-slate-800 px-4 py-2.5 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl flex flex-col md:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <span className="flex h-2 w-2 rounded-full bg-emerald-400 animate-pulse" />
            <span className="text-xs font-bold tracking-wider text-amber-400 uppercase">N-LINK 360 Workspace Simulator</span>
          </div>
          <div className="flex flex-wrap items-center justify-center gap-2">
            {SIMULATOR_USERS.map((simUser) => {
              const isActive = (selectedUser?.role || SIMULATOR_USERS[0].role) === simUser.role;
              return (
                <button
                  key={simUser.role}
                  type="button"
                  onClick={() => setSelectedUser(simUser)}
                  className={`relative rounded-xl px-3.5 py-1.5 text-xs font-bold transition duration-200 outline-none ${
                    isActive
                      ? 'bg-amber-400 text-slate-950 shadow-md ring-2 ring-amber-300'
                      : 'bg-slate-800 text-slate-300 hover:bg-slate-700 hover:text-white'
                  }`}
                  title={`${simUser.fullName} - ${simUser.desc}`}
                >
                  {simUser.label}
                </button>
              );
          ...
          <div className="hidden lg:block text-[11px] text-slate-400 font-medium">
            Active Workspace: <span className="text-amber-300 font-bold">{(selectedUser?.role || SIMULATOR_USERS[0].role) === 'SALES_RECOVERY' ? 'Field Mobile App' : 'Company Backoffice Portal'}</span>
          </div>
        </div>
      </div>
