...
import { Customer, CustomerVisit, Dispatch, InventoryBalance, Invoice, LedgerEntry, PaymentMode, Recovery, SalesOrder, SKU, StockReturn, User } from './types';
...
function AuthenticatedApp({ currentUser, onSignOut }: { currentUser: User; onSignOut: () => Promise<void> }) {
  ...
  const handlePostInvoice = (orderId: string) => {
    ...
    if (result.success) syncStore(); else alert(`Invoice generation failed: ${result.error}`);
  };
  const handleVerifyRecovery = (recoveryId: string) => {
    const result = inMemoryStore.verifyRecovery(recoveryId, currentUser.id);
    if (result.success) syncStore(); else alert(`Recovery verification failed: ${result.error}`);
  };
  const handleBookOrder = (orderData: Partial<SalesOrder>) => { inMemoryStore.createSalesOrder(orderData); syncStore(); };
  const handleRecordRecovery = (data: { customerId: string; amount: number; paymentMode: PaymentMode; instrumentNumber?: string; bankName?: string; remarks?: string }) => {
    ...
    inMemoryStore.recordRecovery({ recoveryNumber: recNumber, customerId: data.customerId, customerName: cust?.companyName || 'Customer', salesUserId: currentUser.id, salesUserName: currentUser.fullName, collectionDate: new Date().toISOString().split('T')[0], amount: data.amount, paymentMode: data.paymentMode, instrumentNumber: data.instrumentNumber, bankName: data.bankName, status: 'PENDING_VERIFICATION', remarks: data.remarks });
    syncStore();
  };
  const handleLogVisit = (visitData: Partial<CustomerVisit>) => { inMemoryStore.logCustomerVisit(visitData); syncStore(); };

  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 antialiased">
      <Header currentUser={currentUser} onSignOut={onSignOut} />
      <main className="min-h-[calc(100vh-4rem)]">
        {activeApp === 'PORTAL' && <CompanyPortal currentUser={currentUser} customers={customers} skus={skus} inventoryBalances={inventoryBalances} salesOrders={salesOrders} invoices={invoices} recoveries={recoveries} ledgerEntries={ledgerEntries} dispatches={dispatches} stockReturns={stockReturns} onPostInvoice={handlePostInvoice} onVerifyRecovery={handleVerifyRecovery} />}
        {activeApp === 'MOBILE_APP' && <div className="px-4 py-5 sm:px-6 lg:px-8"><SalesRecoveryApp currentUser={currentUser} customers={customers} skus={skus} inventoryBalances={inventoryBalances} visits={visits} onBookOrder={handleBookOrder} onRecordRecovery={handleRecordRecovery} onLogVisit={handleLogVisit} /></div>}
      </main>
