...
import { Plus, RefreshCw, CheckCircle2, XCircle, Search, Users, Store, Package, MapPin, ShieldCheck } from 'lucide-react';
...
const tabs: Array<{ id: OperationSubTab; label: string }> = [
  { id: 'COMPANY', label: 'Company / Branches' },
  { id: 'BRANDS_PRODUCTS', label: 'Products / SKUs' },
  { id: 'DEALERS_DISTRIBUTORS', label: 'Dealers / Distributors' },
  { id: 'TARGET', label: 'Targets' },
  { id: 'SALES_TEAM', label: 'Employees / Sales Team' },
...
export const NeumorphicOperationDomain: React.FC<OperationDomainProps> = ({ activeSubTab, setActiveSubTab, currentUser, searchQuery }) => {
  ...
  const refresh = async () => {
    if (!isSupabaseConfigured || !supabase) throw new Error('Supabase production backend is not configured.');
    setLoading(true); setError('');
    try {
      const [app, br, pr, emp, reg, zon, ar, terr, town, route] = await Promise.all([
        loadSupabaseAppData(currentUser),
        supabase.from('branches').select('*').order('name'),
        supabase.from('products').select('*,brands(name)').order('name'),
        supabase.from('employees').select('id,employee_code,full_name,email,mobile,status,territory,role_id').order('full_name'),
        supabase.from('regions').select('*').order('name'),
        supabase.from('zones').select('*,regions(name)').order('name'),
        supabase.from('areas').select('*,zones(name)').order('name'),
        supabase.from('territories').select('*,areas(name)').order('name'),
        supabase.from('towns').select('*,territories(name)').order('name'),
        supabase.from('routes').select('*,towns(name)').order('name'),
      ]);
      const failed = [br,pr,emp,reg,zon,ar,terr,town,route].find(x => x.error);
      if (failed?.error) throw failed.error;
      setData(app); setBranches(br.data || []); setProducts(pr.data || []); setEmployees(emp.data || []);
      setHierarchy({ regions: reg.data || [], zones: zon.data || [], areas: ar.data || [], territories: terr.data || [], towns: town.data || [], routes: route.data || [] });
    } catch (e) { setError(e instanceof Error ? e.message : 'Unable to load live operations data.'); }
  ...
  const submitCustomer = async (e: React.FormEvent) => {
    ...
    try {
      await registerCustomerPending({ customerType: customerForm.type, businessName: customerForm.name, ownerName: customerForm.owner, mobile: customerForm.mobile, address: customerForm.address, city: customerForm.city, proposedCreditLimit: Number(customerForm.creditLimit), proposedCreditDays: Number(customerForm.creditDays) });
      setCustomerForm({ type:'DEALER',name:'',owner:'',mobile:'',address:'',city:'',creditLimit:'',creditDays:'' }); setShowCustomerForm(false); await refresh();
  ...
  return (
    <section className="space-y-5 p-4 sm:p-6">
      ...
      {loading ? <div className="nm-flat rounded-2xl p-10 text-center text-sm font-bold text-slate-500">Loading live production data…</div> : <>
        ...
        {activeSubTab === 'HIERARCHY' && <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">{Object.entries(hierarchy).map(([k,v])=><div key={k} className="nm-flat rounded-2xl p-4"><div className="text-[10px] uppercase font-bold text-slate-500">{k}</div><div className="text-2xl font-black text-slate-800">{v.length}</div></div>)}</div>}
        {activeSubTab === 'APPROVALS' && <div className="space-y-5"><ApprovalSection title="Dealer / Distributor Registrations" count={pendingCustomers.length}>{pendingCustomers.map(c=><div key={c.id} className="nm-flat rounded-2xl p-4 flex flex-wrap items-center justify-between gap-3"><div><div className="font-black">{c.companyName}</div><div className="text-xs text-slate-500">{c.customerCode} · {c.type} · {c.city}</div></div>{admin?<div className="flex gap-2"><button onClick={()=>void approveCustomer(c.id,c.customerCode)} className="nm-btn-primary px-3 py-2 rounded-xl text-xs font-bold"><CheckCircle2 className="inline w-4 h-4 mr-1"/>Approve</button><button onClick={()=>void rejectCustomer(c.id)} className="nm-btn px-3 py-2 rounded-xl text-xs font-bold text-rose-700"><XCircle className="inline w-4 h-4 mr-1"/>Reject</button></div>:null}</div>)}</ApprovalSection><ApprovalSection title="Sales Orders" count={pendingOrders.length}>{pendingOrders.map(o=><div key={o.id} className="nm-flat rounded-2xl p-4 flex flex-wrap items-center justify-between gap-3"><div><div className="font-black">{o.orderNumber} · {o.customerName}</div><div className="text-xs text-slate-500">{o.status} · PKR {o.totalAmount.toLocaleString()}</div></div>{admin?<div className="flex gap-2"><button onClick={()=>void approveSalesOrder(o.id)} className="nm-btn-primary px-3 py-2 rounded-xl text-xs font-bold">Approve</button><button onClick={()=>void rejectSalesOrder(o.id)} className="nm-btn px-3 py-2 rounded-xl text-xs font-bold text-rose-700">Reject</button></div>:null}</div>)}</ApprovalSection><ApprovalSection title="Recoveries" count={pendingRecoveries.length}>{pendingRecoveries.map(r=><div key={r.id} className="nm-flat rounded-2xl p-4 flex flex-wrap items-center justify-between gap-3"><div><div className="font-black">{r.recoveryNumber} · {r.customerName}</div><div className="text-xs text-slate-500">PKR {r.amount.toLocaleString()} · {r.paymentMode}</div></div>{admin?<div className="flex gap-2"><button onClick={()=>void verifyRec(r.id)} className="nm-btn-primary px-3 py-2 rounded-xl text-xs font-bold">Verify</button><button onClick={()=>void rejectRec(r.id)} className="nm-btn px-3 py-2 rounded-xl text-xs font-bold text-rose-700">Reject</button></div>:null}</div>)}</ApprovalSection></div>}
      </>}
