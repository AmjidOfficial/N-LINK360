...
interface OperationDomainProps {
  ...
  onApproveCustomer?: (id: string, approverName?: string) => void;
  onRejectCustomer?: (id: string, reason?: string) => void;
}
...
export const NeumorphicOperationDomain: React.FC<OperationDomainProps> = ({
  ...
  useEffect(() => {
    if (customers && customers.length > 0) {
      setDealers(customers);
      const pendingFromCust = customers
        .filter((c) => c.approvalStatus === 'PENDING_APPROVAL' || c.status === 'PENDING_APPROVAL' || (!c.isActive && c.approvalStatus !== 'REJECTED'))
        .map((c) => ({
          id: c.id,
          businessName: c.companyName || c.name || 'Commercial Partner',
          ownerName: c.contactPerson || c.ownerName || 'Key Contact',
          contactNumber: c.phone || c.mobile || '—',
          cnic: c.cnic || '—',
          address: c.address || '',
          city: c.town || c.city || 'Punjab',
          region: c.region || 'Punjab Central',
          type: c.type || c.customerType || 'DEALER',
          proposedCreditLimit: c.creditLimit || 500000,
          proposedCreditDays: c.creditDays || 15,
          proposedOpeningBalance: c.openingBalance || 0,
          additionalNotes: c.notes || 'Submitted via Mobile / Dynamic Registration Portal',
          salesUserId: c.salesUserId || c.submittedById || '',
          salesUserName: c.salesUserName || c.submittedBy || 'Field Officer',
          submittedAt: c.createdAt ? new Date(c.createdAt).toLocaleDateString() : 'Recent',
          status: c.approvalStatus || 'PENDING_APPROVAL',
          assignedCode: c.customerCode,
        }));
      if (pendingFromCust.length > 0) {
        setPendingRegistrations((prev) => {
          const prevMap = new Map<string, any>(prev.map((p) => [p.id, p]));
          for (const item of pendingFromCust) {
  ...
  const handleApproveRegistration = async (regId: string, customApproverEmail?: string) => {
    ...
    const newCode = reg.assignedCode && !reg.assignedCode.startsWith('CUST-REG')
      ? reg.assignedCode
      : (reg.type === 'DISTRIBUTOR' ? `DST-2026-${Math.floor(100000 + Math.random() * 900000)}` : `DLR-2026-${Math.floor(100000 + Math.random() * 900000)}`);

    try {
      if (onApproveCustomer) {
        await onApproveCustomer(regId, approverEmail);
      } else {
        await approveCustomerRegistration(regId, newCode, approverEmail);
      }
      setPendingRegistrations((prev) =>
      ...
      const newDealerRecord = {
        ...
        bankIban: 'PK-CONFIRMED',
      };
      setDealers((prev) => [newDealerRecord, ...prev.filter((d) => d.id !== regId && d.id !== newCode)]);
      showApprovalToast(`Partner Application "${reg.businessName}" Approved! Assigned Code: ${newCode} (Authorized by ${approverEmail})`);
    } catch (err: any) {
      console.warn('Approval note:', err);
      setPendingRegistrations((prev) =>
        prev.map((item) => (item.id === regId ? { ...item, status: 'APPROVED', assignedCode: newCode } : item))
      );
      showApprovalToast(`Partner Application "${reg.businessName}" Approved! Code: ${newCode}`);
    }
  };
  ...
  const handleRejectRegistration = async (regId: string) => {
    ...
    try {
      if (onRejectCustomer) {
        ...
        await rejectCustomerRegistration(regId, 'Credit policy non-compliance / incomplete territory documentation', approverEmail);
      }
      setPendingRegistrations((prev) =>
      ...
      console.warn('Rejection note:', err);
      setPendingRegistrations((prev) =>
        prev.map((item) => (item.id === regId ? { ...item, status: 'REJECTED' } : item))
      );
      showApprovalToast(`Application ${regId} status updated to Rejected.`);
    }
  };
  ...
  const handleSaveDynamicDealer = (dealerData: any) => {
    ...
    if (modalType === 'ADD_DEALER') {
      const newDlr = {
        ...dealerData,
        id: dealerData.id || `DLR-${100 + dealers.length + 1}`,
      };
      setDealers((prev) => [newDlr, ...prev]);
      if (onAddCustomer) onAddCustomer(newDlr);
    } else if (modalType === 'EDIT_DEALER' && selectedDealer) {
      const updated = { ...selectedDealer, ...dealerData };
      setDealers((prev) =>
        prev.map((d) => (d.id === selectedDealer.id ? updated : d))
      );
  ...
  const handleSaveDealer = (e: React.FormEvent) => {
    ...
    if (modalType === 'ADD_DEALER') {
      ...
      const updated = { ...selectedDealer, ...dealerForm };
      setDealers((prev) => prev.map((d) => (d.id === selectedDealer.id ? updated : d)));
      if (onUpdateCustomer) onUpdateCustomer(updated);
    }
    setModalType(null);
  };
                  ...
                  <div key={reg.id} className="nm-inset p-4 rounded-2xl bg-white space-y-3 text-xs">
                    ...
                    <div className="flex gap-2 pt-2 border-t border-slate-100">
                      <button
                        ...
                        onClick={() => {
                          if (onRejectCustomer) {
                            onRejectCustomer(reg.id, 'Declined by Head Office Admin');
                          }
                      ...
                      <button
                        ...
                        onClick={() => {
                          const newDealer = {
                            id: reg.id || `DLR-${Math.floor(104 + Math.random() * 900)}`,
                            name: reg.businessName || reg.name || 'Commercial Account',
                            companyName: reg.businessName || reg.name || 'Commercial Account',
                            customerType: reg.type || reg.customerType || 'DEALER',
                            region: reg.region || 'Punjab Central',
                            area: `${reg.city || 'Lahore'} Division`,
                            territory: reg.address || reg.territory || '',
                            ...
                            town: reg.city || reg.town || 'Lahore',
                            city: reg.city || reg.town || 'Lahore',
                            cnic: reg.cnic || '',
                            contactPerson: reg.ownerName || reg.contactPerson || '',
                            phone: reg.contactNumber || reg.phone || '',
                            address: reg.address || '',
                            creditLimit: Number(reg.proposedCreditLimit) || 1000000,
                            creditDays: Number(reg.proposedCreditDays) || 30,
                            currentBalance: Number(reg.proposedOpeningBalance) || 0,
                            status: 'NORMAL',
                            approvalStatus: 'APPROVED',
                            isActive: true,
                          };
                          ...
                          if (onApproveCustomer) {
                            onApproveCustomer(newDealer.id, currentUser.fullName);
                          } else if (onAddCustomer) {
                            onAddCustomer(newDealer);
                          }
                          setPendingRegistrations(prev => prev.filter(p => p.id !== reg.id));
                          alert(`Authorization Complete! "${reg.businessName || reg.name}" has been successfully activated.`);
                        }}
