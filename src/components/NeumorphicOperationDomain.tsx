...
const tabs: Array<{ id: OperationSubTab; label: string }> = [
  { id: 'COMPANY', label: 'Company / Branches' }, { id: 'BRANDS_PRODUCTS', label: 'Products / SKUs' },
  { id: 'DEALERS_DISTRIBUTORS', label: 'Dealers / Distributors' }, { id: 'TARGET', label: 'Targets' },
  { id: 'SALES_TEAM', label: 'Employees / Sales Team' }, { id: 'HIERARCHY', label: 'Hierarchy' }, { id: 'APPROVALS', label: 'Approvals' },
];
  ...
  const refresh = async () => {
    ...
    try {
      const [app, br, pr, emp, reg, zon, ar, terr, town, route] = await Promise.all([
        loadSupabaseAppData(currentUser), client.from('branches').select('*').order('name'), client.from('products').select('*,brands(name)').order('name'),
        client.from('employees').select('id,employee_code,full_name,email,mobile,status,territory,role_id').order('full_name'),
        client.from('regions').select('*').order('name'), client.from('zones').select('*,regions(name)').order('name'), client.from('areas').select('*,zones(name)').order('name'),
        client.from('territories').select('*,areas(name)').order('name'), client.from('towns').select('*,territories(name)').order('name'), client.from('routes').select('*,towns(name)').order('name'),
      ]);
