...
export const VisitsMapView: React.FC<VisitsMapViewProps> = ({ visits }) => {
  ...
  useEffect(() => {
    if (!isLeafletReady || !mapContainerRef.current || visits.length === 0) return;

    const L = (window as any).L;
    if (!L) return;

    // Filter valid coordinates
    const validVisits = visits.filter(v => typeof v.latitude === 'number' && typeof v.longitude === 'number' && v.latitude !== 0 && v.longitude !== 0);
    ...
    sortedVisits.forEach((vis, idx) => {
      ...
      const customIcon = L.divIcon({
        html: `
          <div class="relative flex items-center justify-center">
            <div class="absolute w-7 h-7 bg-amber-500 rounded-full animate-ping opacity-25"></div>
            <div class="w-6 h-6 bg-slate-900 border-2 border-amber-400 text-[10px] text-amber-400 font-bold font-mono rounded-full flex items-center justify-center shadow-lg">
              ${idx + 1}
            </div>
          </div>
        `,
      ...
      const popupContent = `
        <div class="p-2 font-sans space-y-1 w-52 text-slate-800">
          ...
          <h4 class="font-bold text-slate-950 text-xs border-b pb-1">${vis.customerName}</h4>
          ...
          <div class="text-[10px] italic bg-slate-50 p-1 border rounded text-slate-600 mt-1">"${vis.notes || 'Routine check-in'}"</div>
          ${vis.photoUrl ? `
  ...
  return (
    <div className="bg-slate-50 rounded-xl border border-slate-200 overflow-hidden shadow-xs grid grid-cols-1 lg:grid-cols-3">
      
      {/* MAP CANVAS */}
      <div className="lg:col-span-2 relative h-80 lg:h-[400px] bg-slate-100 flex flex-col justify-end">
        {mapError && (
          <div className="absolute inset-0 z-[1000] flex items-center justify-center p-6 bg-slate-900/10 backdrop-blur-xs text-center">
            <div className="bg-white p-4 rounded-xl border shadow-md space-y-2 max-w-sm">
              ...
              <button 
                ...
                className="px-3 py-1 bg-slate-900 text-white rounded text-[10px] font-bold"
              >
                Dismiss
              </button>
            </div>
        ...
        <div className="absolute bottom-3 left-3 z-[1000] bg-slate-900/90 text-amber-400 font-mono text-[10px] font-bold px-2 py-1 rounded-md shadow-lg border border-slate-800/80">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-1">
                      <strong className={`truncate block text-xs ${isSelected ? 'text-white' : 'text-slate-900'}`}>{vis.customerName}</strong>
                      <span className={`text-[9px] font-mono shrink-0 font-medium ${isSelected ? 'text-amber-400' : 'text-slate-500'}`}>
