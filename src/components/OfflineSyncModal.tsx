      ...
      <div className="flex w-full max-w-2xl flex-col rounded-2xl bg-white shadow-2xl overflow-hidden border border-slate-200">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 bg-slate-900 px-6 py-4 text-white">
          <div className="flex items-center gap-3">
            <div className={`flex h-9 w-9 items-center justify-center rounded-xl font-black ${isOnline ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'}`}>
              {isOnline ? <Wifi className="h-5 w-5" /> : <WifiOff className="h-5 w-5" />}
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-bold text-white">Offline Engine & Queue Manager</h3>
                <span className={`rounded-md px-2 py-0.5 text-[10px] font-extrabold ${isOnline ? 'bg-emerald-400/20 text-emerald-300 border border-emerald-400/30' : 'bg-rose-400/20 text-rose-300 border border-rose-400/30'}`}>
        ...
        {/* Metric Summary */}
        <div className="grid grid-cols-3 gap-2 bg-slate-50 p-4 border-b border-slate-200">
        ...
        <div className="max-h-[45vh] overflow-y-auto p-4 space-y-2.5">
          {queue.length === 0 ? (
            ...
            queue.map((item) => (
              <div
                key={item.id}
                className="flex items-center justify-between p-3 rounded-xl border border-slate-200 bg-white hover:bg-slate-50/80 shadow-2xs text-xs"
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-slate-900 uppercase">{item.module}</span>
                      <span
        ...
        <div className="flex items-center justify-between border-t border-slate-200 bg-slate-50 px-6 py-3.5">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-5 py-2 text-xs font-bold text-white shadow-md hover:bg-slate-800 disabled:opacity-50"
          >
