      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            ...
            <span className="text-2xl font-bold font-mono text-slate-900 mt-1">{productionBatches.length}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 bg-white rounded-xl border border-slate-200 p-5 shadow-sm space-y-4">
          ...
          <div className="space-y-3">
            {factoryStocks.map(f => (
              <div key={f.skuId} className="p-3 bg-slate-50 rounded-lg border border-slate-100 flex justify-between items-center text-xs">
                <div>
                ...
                <div className="text-right">
                  <span className="text-sm font-bold font-mono text-slate-900 block">{f.currentFG.toLocaleString()} Pcs</span>
          ...
          <div className="pt-2">
            <button
              ...
              className="w-full py-2 bg-slate-900 hover:bg-slate-800 text-amber-400 font-bold text-xs rounded-lg transition-colors flex items-center justify-center gap-2 shadow-sm"
            >
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-100 font-mono">
                {productionBatches.map(b => (
                  <tr key={b.id} className="hover:bg-slate-50/50 font-sans text-[11px]">
                    <td className="py-2 px-2 font-mono font-bold text-slate-900">{b.batchCode}</td>
                    ...
                    <td className="py-2 px-2 text-right font-mono font-bold text-slate-900">{b.quantity.toLocaleString()} Pcs</td>
                    ...
                    <td className="py-2 px-2 text-center">
                      {b.status === 'PENDING_QA' ? (
                        <button
                          ...
                          className="px-2 py-1 bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-[10px] rounded transition-colors"
                        >
                          Approve QA
                        </button>
                      ) : (
                        <span className="text-emerald-600 text-[10px] font-semibold flex items-center justify-center gap-0.5 font-sans">
                          ✓ QA Pass
                        </span>
                      )}
                    </td>
            ...
            <tbody className="divide-y divide-slate-100 font-mono text-xs">
              {transfers.map(t => (
                <tr key={t.id} className="hover:bg-slate-50/50 font-sans">
                  <td className="py-2.5 px-3 font-mono font-bold text-slate-900">{t.transferCode}</td>
                  ...
                  <td className="py-2.5 px-3 text-right font-mono font-black text-slate-900">{t.quantity.toLocaleString()} Pcs</td>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full overflow-hidden border border-slate-100">
            <div className="bg-slate-900 text-white px-6 py-4 flex justify-between items-center">
              <h3 className="font-extrabold text-sm flex items-center gap-2">
                <Boxes className="w-5 h-5 text-amber-400" /> Log Factory Production Output
              </h3>
            ...
            <form onSubmit={handleProductionSubmit} className="p-6 space-y-4 text-xs">
              ...
              <div className="pt-3 flex gap-2 border-t">
                ...
                <button type="submit" className="flex-1 py-2 bg-slate-900 hover:bg-slate-800 text-amber-400 font-bold rounded-lg shadow">Post Batch Output</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showTransferModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full overflow-hidden border border-slate-100">
            <div className="bg-slate-900 text-white px-6 py-4 flex justify-between items-center">
              <h3 className="font-extrabold text-sm flex items-center gap-2">
                <ArrowRightLeft className="w-5 h-5 text-amber-400" /> Dispatch FG to Depot
              </h3>
            ...
            <form onSubmit={handleTransferSubmit} className="p-6 space-y-4 text-xs">
              ...
              <div className="pt-3 flex gap-2 border-t">
                ...
                <button type="submit" className="flex-1 py-2 bg-slate-900 hover:bg-slate-800 text-amber-400 font-bold rounded-lg shadow">Confirm Transfer</button>
              </div>
            </form>
          </div>
        </div>
