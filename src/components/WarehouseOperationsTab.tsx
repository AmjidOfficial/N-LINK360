      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b pb-4">
          ...
          <button
            ...
            className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-amber-400 font-bold text-xs rounded-lg transition-colors flex items-center gap-1.5 shadow-sm"
          >
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50 text-slate-600 font-bold border-b">
              <tr>
                ...
                <th className="py-2.5 px-3 text-right text-emerald-600">Stock In (+)</th>
                ...
                <th className="py-2.5 px-3 text-right bg-slate-100 font-extrabold text-slate-900 border-l border-r">Current Stock</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 font-mono text-xs">
              {filteredBalances.map(b => (
                <tr key={b.skuId} className="hover:bg-slate-50/50">
                  ...
                  <td className="py-3 px-3 text-right bg-indigo-50/50 font-black text-slate-900 border-l border-r font-mono text-sm">
              ...
              {inventoryTransactions.slice(0, 15).map(tx => {
                ...
                return (
                  <tr key={tx.id} className="hover:bg-slate-50/50">
                    <td className="py-2 px-3 font-mono font-bold text-slate-900">{tx.transactionNumber || 'TX-NEW'}</td>
                    ...
                    <td className={`py-2 px-3 text-right font-bold ${isDeduction ? 'text-rose-600' : 'text-emerald-600'}`}>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full overflow-hidden border border-slate-100">
            <div className="bg-slate-900 text-white px-6 py-4 flex justify-between items-center">
              <h3 className="font-extrabold text-sm flex items-center gap-2">
                <Edit3 className="w-5 h-5 text-amber-400" /> Log Custom Inventory Event
              </h3>
            ...
            <form onSubmit={handleAdjustmentSubmit} className="p-6 space-y-4 text-xs">
              ...
              <div className="pt-3 flex gap-2 border-t">
                ...
                <button type="submit" className="flex-1 py-2 bg-slate-900 hover:bg-slate-800 text-amber-400 font-bold rounded-lg shadow">Post Transaction</button>
              </div>
