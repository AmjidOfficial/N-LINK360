      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-8 bg-white rounded-xl border border-slate-200 p-5 shadow-sm space-y-5">
          ...
          {selInvoice && selInvoice.status !== 'CANCELLED' ? (
            <form onSubmit={handlePostNote} className="p-4 bg-slate-50/50 rounded-xl border border-dashed border-slate-300 space-y-4 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full p-1.5 border rounded-lg font-bold text-slate-900 bg-white"
                  >
              ...
              <div className="pt-2 flex justify-end">
                <button
                  type="submit"
                  className="px-5 py-2 bg-slate-900 hover:bg-slate-800 text-amber-400 font-extrabold rounded-lg shadow-md transition-all"
                >
                ...
                <tbody className="divide-y divide-slate-100">
                  {auditLogs.filter(l => l.action.includes('NOTE') || l.action.includes('CANCEL')).map(log => (
                    <tr key={log.id} className="hover:bg-slate-50 text-xs">
                      ...
                      <td className="py-2 px-2 text-right font-bold text-slate-900">PKR {log.amount?.toLocaleString()}</td>
            ...
            <div className="space-y-3 pt-1">
              {pendingApprovalOrders.length === 0 ? (
                <div className="p-3 bg-emerald-50 rounded-lg text-center border border-emerald-100">
                  ...
                  <span className="text-[10px] text-emerald-600 block mt-0.5">No orders are blocked by credit policy.</span>
                </div>
              ) : (
                pendingApprovalOrders.map(o => {
                  ...
                  return (
                    <div key={o.id} className="p-3.5 bg-slate-50 border border-slate-200 rounded-lg text-xs space-y-3">
                      <div className="flex justify-between items-start">
                        <div>
                          <span className="font-mono text-indigo-700 font-bold block">{o.orderNumber}</span>
                          <span className="font-bold text-slate-900 block mt-0.5">{o.customerName}</span>
                        </div>
                      ...
                      <button
                        ...
                        className="w-full py-2 bg-slate-900 hover:bg-slate-800 text-amber-400 font-bold text-xs rounded-lg transition-colors flex items-center justify-center gap-1.5 shadow"
                      >
